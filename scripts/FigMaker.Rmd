...
CompFig(feature_c = "../data/clean/binCover.txt",
        ...
        mapfile = "../data/clean/mapfile.txt",
        flabel = BinQT)
ggsave("../figs/Bin_heatmap.png", width = 18, height = 25, units = "cm")
...
CompFig(feature_c = "../data/clean/MAGCover.txt",
        ...
        mapfile = "../data/clean/mapfile.txt",
        flabel = BinQT)
ggsave("../figs/MAG_heatmap.png", width = 18, height = 20, units = "cm")
...
# 1. Plot the basic tree
p <- ggtree(tree) + 
  geom_tiplab(size = 2, align = TRUE) #+ 
  #hexpand(.1, side = 1) # Add space for labels

# 2. Add the heatmap
# 'data' must be a data frame or matrix with row names matching tree tip labels
ph <- gheatmap(p, table_wide, 
               offset = 0.5, 
               width = 2, 
               colnames_angle = 90, 
               colnames_offset_y = -0.1,
               font.size = 2) +
  # Custom gradient: Low to High
  scale_fill_gradient(low = "#f7f7f7", 
                      high = "#0571b0", 
                      name = "Coverage",
                      na.value = "#f7f7f7") # Treat NAs as the lowest color
